...
FRAG_BINS = 20             # 阶段 1 片段大小：20 个 bin，即 20 Mb
...
# 混合哈希产生的折 id：train = 0..5，val = 6..7，test = 8..9。
N_FOLD = 10
TRAIN_FOLDS = (0, 1, 2, 3, 4, 5)
VAL_FOLDS = (6, 7)
TEST_FOLDS = (8, 9)

# 只在 VALIDATION 折上筛选 distance exponent（绝不使用 test）。
...
def safe_name(chrom, idx):
    """返回 `chrom` 的第 `idx` 个 copy 所用的 FDG 轨迹名。

    hickit 不能在同一文件中同时使用染色体名 `chr1`（haploid consensus）和 paired copy，因此双拷贝运行使用专用名称。
    """
