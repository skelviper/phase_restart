#!/usr/bin/env python
"""P9016 去单倍型列二倍体重建项目的轻量驱动器。

    run.py prepare                   将 phase 列剥离到 inputs/
    run.py stage1 [options]          Stage 1：fragment-swap 实验
    run.py split --genome [options]  Stage S0：全基因组 reference points
    run.py reevaluate [options]      对已有 S0 坐标执行经哈希验证的 metrics/figures
    run.py reconstruct [options]     正式的无 phase Reconstruction V1 training

Stage 1 检验无标签 held-out score 能否把染色体范围内的一致 split 与“局部正确但全局错接”的 split 区分开。
每个候选只在 training fold 上重新拟合，并在同一个 held-out fold 上评分。这是 oracle instrument 检查，不是 blind recovery。
"""
import argparse
...
def json_ready(value):
    """将正式结果文件写成严格 JSON（不允许 IEEE NaN token）。"""
...
class Candidate:
    def __init__(self, cid, family, tracks, assign=None, structs=None, tb=None,
        ...
        self.id = cid
        self.family = family        # "blind" | "null" | "oracle" 三种 family
        self.tracks = tracks        # track name 列表（1 或 2 个）
        self.assign = assign        # 覆盖 `rec` 的 label 数组，或 None
        self.structs = structs
        self.tb = tb                # 逐 bin flip mask，或 None
        self.rule = rule
        self.note = note
        self.rec = rec              # 该 split 定义所用的 (p1, p2, truth_label)


def fit_candidate(cid, chrom, p1, p2, assign, workdir, coordsdir, seed=1):
    """执行一次 FDG 拟合并返回解析后的结构。"""
...
def evaluate(cand, chrom, vs, ts, n_bins):
    """按 validation 选择指数，并为每个候选生成 held-out 读出。"""
...
def cmd_stage1(args):
    ...
    # Reporting rule 2：oracle 只能由完全标注的 contacts 构建。
    # 因此每个 label-based 候选都使用同一组记录（phased train records），只在 labels 上不同。
    # blind controls 使用更大的 all-record train set，因此是更严格的 controls。
    ph_tr = tr & (lb >= 0)
    ...
    o_b1, o_b2 = I1[ph_tr], I2[ph_tr]        # 这里是 bin，不是 fragment 索引
    if args.ph_subsample and args.ph_subsample < n_ph_tr:
        # 深度控制：除完全 phase 标注的训练记录数量外，其余设置保持不变，
        # 这样可以把 chrX 与相同深度的 chr1 比较，而不是与另一条染色体比较。
    ...
    # null：在同一组已 phase 标注的记录上进行多个独立同分布的 50/50 划分。它们无标签且结构上等价，
    # 因而 rho 的离散程度正是 swap effect 必须超过的“重新分配波动”。
    ...
    for kind, rule in patterns:
        ...
        # gauge complement（翻转其余每个 bin 的 label）与 copyA/copyB 交换后的假设相同。
        # 对 `rewire` 它实际上产生同一个 assignment，因此只有 `relabel` 需要额外拟合。
        if rule == "relabel" and 0 < tb.sum() < n_bins:
            add_pattern(cid + "_gc", ~tb, rule, tag="   [gauge complement]")

    # 独立 oracle null：FDG 输出在不同 `-s` seed 下按位一致（已验证），
    # 因此拟合之间的变化只能来自数据。将已 phase 标注的训练记录随机分成两个不重叠的部分，
    # 用来估计 swap difference 必须超过的拟合噪声下限。
    ...
    # ---- 5. 评分 -----------------------------------------------------
    # NaN policy（已声明）：如果某候选自己的拟合记录集中某个 bin 没有 contact，
    # 其 FDG 输出就不会有该 bin 的 bead，因此该距离确实未定义。每次比较只使用涉及两候选的
    # 最大公共集合，并让每行报告自己的 n。候选之间不取全局交集，否则一个辅助诊断拟合
    # 会悄悄缩小其他所有结果的样本。
    ...
    TS = ts
    # STRATUM_IDX 保存索引数组而非布尔 mask。不要用 `&` 把两者合并：
    # numpy 会把 bool 转成 0/1，`arange(n) & mask` 会悄悄变成 `arange(n) & 1`，也就是
    # 按行号奇偶取值。这与已废弃的 `(i+j) % 2` fold hash 属于同类缺陷；应使用
    # `common(*cands)` 构造布尔 mask。
    ...
    def row_masks(c):
        """返回 `{stratum: index array}`，数组只包含该候选的有限值。"""
        out = {}
    ...
    def common(*cs):
        """返回 BOOLEAN mask：这些候选在 test fold 上均有有限值的 pair。"""
    ...
    # ---- 6. 针对 oracle 的 paired bootstrap ---------------------------
    ...
    for c in cands:
        ...
        # delta 与 CI 必须落在同一个 universe 上，否则点估计和区间回答的是不同问题。
    ...
    # ---- 6d. 仅比较每个候选自身破坏的受影响 bin pairs：
    #          rho(oracle) 与 rho(candidate)，并在相同 pairs 上配对 null。
    masked = {}
    ...
    # ---- 7. boundary relaxation；所有候选使用相同 budget -------------
    relax = {}
    if args.relax:
    ...
    # 结构与 reference 的 rank correlation；按几何选择最佳 swap
    refcorr = {}
    for c in cands:
        row = {}
    ...
    # ---- 9b. selection test：无标签 criterion 是否把染色体范围内的一致
    #          split 排在第一位？ --------------------------------------
    selection = {}
    log("--- selection test ---")
    families = {
...
def make_plots(res, outdir, cands, ts, chrom):
    """三英寸面板、300 dpi：剂量响应、分层、掩码效应和距离图。"""
    from pr import figs
...
def relax_candidate(c, chrom, inband, outdir, rounds, vs, ts, gate=None):
    """运行受支持的一轮 boundary E-step 和 FDG refit。"""
    if rounds != 1:
...
def cmd_split(args):
    ...
    lab = refeval.single_label(a1, a2)          # 仅用于 R1：两端必须一致
...
# --------------------------------------------------------------------------
# reevaluate -- 仅处理已冻结的现有坐标
# --------------------------------------------------------------------------
def cmd_reevaluate(args):
    """在不启动 FDG 拟合的情况下，对已验证的 S0 坐标重新评分。"""
...
# --------------------------------------------------------------------------
# reconstruct -- 仅执行正式 V1 training
# --------------------------------------------------------------------------
def cmd_reconstruct(args):
    """在新的正式目录中运行已发布的无 phase V1 训练路径。"""
    target_threads = str(args.threads)
    marker = "_P9016_RECONSTRUCT_THREADS"
    # `run.py` 为兼容旧命令导入 NumPy。重新执行一次，使选定的 thread cap
    # 在实际 training process 导入 NumPy/BLAS 之前生效。
