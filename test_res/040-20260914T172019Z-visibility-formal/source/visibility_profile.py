...
class VisibilityGPUObjective:
    ...
    def __init__(self, data: contact_model.AggregatedContacts, mode: str,
        ...
        self._last_physical_gradient: np.ndarray | None = None
        self._counters = {
            "outer_evaluations": 0,
    ...
    def evaluate(self, theta: np.ndarray, need_gradient: bool = True):
        ...
        gradient_p_physical = count_gradient_p + p_prior_derivative
        self._last_physical_gradient = torch.cat((combined_x.reshape(-1), gradient_p_physical.reshape(1))).detach().cpu().numpy().astype(np.float64, copy=True)
    ...
    def physical_gradient(self) -> np.ndarray | None:
        """返回最终 value+gradient 请求对应的 (dF/dX,dF/dp) 梯度。"""
        return None if self._last_physical_gradient is None else self._last_physical_gradient.copy()
