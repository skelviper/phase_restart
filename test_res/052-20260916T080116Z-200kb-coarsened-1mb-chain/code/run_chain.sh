#!/usr/bin/env bash
# 052 新链：40Mb -> 10Mb -> 5Mb -> 2Mb -> 1Mb -> 500kb -> 200kb
# 每层一个独立进程；任一层非零退出立即停止，不续跑、不复用别的条件的起点。
set -uo pipefail
cd "$(dirname "$0")/.."
RUN_DIR="$(pwd)"
PY=python
...
declare -A PREV=( ["40Mb"]="" ["10Mb"]="coords/new-chain/40Mb.npz" ["5Mb"]="coords/new-chain/10Mb.npz" \
                  ["2Mb"]="coords/new-chain/5Mb.npz" ["1Mb"]="coords/new-chain/2Mb.npz" \
                  ["500kb"]="coords/new-chain/1Mb.npz" ["200kb"]="coords/new-chain/500kb.npz" )
CHAIN=(40Mb 10Mb 5Mb 2Mb 1Mb 500kb 200kb)
...
for stage in "${CHAIN[@]}"; do
  ...
  started=1
  prev="${PREV[$stage]}"
  echo "=== stage $stage (prev='${prev:-root}') start $(date -u +%Y-%m-%dT%H:%M:%SZ)"
  args=(code/run_stage_052.py --stage "$stage")
