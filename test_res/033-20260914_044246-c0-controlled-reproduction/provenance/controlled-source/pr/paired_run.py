"""One-layer paired runner for the post-020 allele sensitivity variants.

This runner intentionally owns no native initialization.  Callers provide three
already materialized, paired physical starts (for example from a separate
explicit full-grid native bridge).  All five variants use one 1 Mb objective
layer and one caller-supplied L-BFGS-B budget.  No budget is chosen here.
"""
...
class PairedRunError(RuntimeError):
    """Raised when a paired-run contract is invalid before or during fitting."""


@dataclass(frozen=True)
class FitConfig:
    """Explicit one-layer solver settings; maxiter and maxfun have no defaults."""
...
@dataclass(frozen=True)
class PairedStart:
    """One physical start shared byte-for-byte by all model variants."""
    ...
    def __post_init__(self) -> None:
        ...
        # q_from_p provides the exact same strict bounded interval as V1 pack().
...
class PairedFitResult:
    """Numerical one-layer result with explicit domain and map audit fields."""
...
def save_paired_start(path: str | Path, start: PairedStart) -> Path:
    """Write one physical start without changing it or adding a hidden rescale."""
    path = Path(path)
...
def load_paired_start(path: str | Path, start_id: str | None = None) -> PairedStart:
    """Load a previously materialized physical start with no native/reference access."""
    path = Path(path)
...
def _validate_start_for_design(data: contact_model.AggregatedContacts,
    ...
    # C0 and C2-map are both present, so every shared paired start must be valid
    # for the strict unit-ball variants even when C2-free itself could accept more.
...
def plan_paired_runs(data: contact_model.AggregatedContacts,
    ...
    """Create an auditable 5-by-N plan without evaluating or optimizing anything."""
...
def run_one_fit(data: contact_model.AggregatedContacts, start: PairedStart,
    ...
    """Run one explicitly budgeted 1 Mb fit; no defaults or coordinate clipping."""
...
def run_paired(data: contact_model.AggregatedContacts,
    ...
    """Run the requested paired variants with one shared explicit solver config."""
...
def select_best_start(results: Sequence[PairedFitResult],
    ...
    """Select within one model using a finite count window and stable start id."""
...
def write_coordinates(path: str | Path, data: contact_model.AggregatedContacts,
    ...
    """Write a full-grid coordinate file with model-specific validation and no clip."""
    path = Path(path)
    values = np.asarray(coordinates, dtype=np.float64)
    if values.shape != (2, data.n_loci, 3):
