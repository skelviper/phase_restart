"""Reconstruction V1 的冻结预算 P3 合成校准。

本模块有三个有意分开的阶段：

* ``prepare`` 只读取已核验的 SNP-free 染色体头和冻结的计数总量，写出无标签的 synthetic 层输入及隔离的 truth；
* ``worker`` 只接收一个 condition 的无标签 counts/exposure 和一份独立初始化，绝不接收 truth path；
* ``evaluate`` 在加载隔离的 synthetic truth 进行 R2/R3 风格诊断前核验候选坐标哈希。

它不打开 P9016 phase 列、reference 坐标、oracle artifact 或生产初始化坐标。
"""
...
def verify_frozen_run_manifest(run_dir: str | Path) -> dict:
    """若注册的 source 或 protocol 发生漂移，拒绝正式阶段。"""
...
def header_templates() -> dict[int, Any]:
    """只读取已核验的 SNP-free header，绝不读取其 body records。"""
...
def _chain_offsets(rng: np.random.Generator, n_loci: int, l0: float,
    ...
    """围绕 territory center 生成一条有界 chain。"""
...
def generate_truth(template, seed: int, same_shape: bool) -> tuple[np.ndarray, dict]:
    """生成包含完整 header 的 40-track synthetic polymer geometry。"""
...
def generation_rates(template, coordinates: np.ndarray, exposure: np.ndarray,
    ...
    """使用预注册 synthetic kernel 生成无 label 的 full-E rates。"""
...
def coarsen_from_final(fine, coarse_template, exposure_strategy: str):
    """将 1 Mb synthetic aggregates 精确映射到更粗的完整 header grid。"""
...
def prepare_run(run_dir: str | Path) -> dict:
    """在任何 worker optimization 开始前创建全部 synthetic artifacts。"""
    run_dir = Path(run_dir)
...
def run_worker(run_dir: str | Path, condition: str) -> dict:
    """拟合一个 condition，不加载任何 truth artifact。"""
    ...
    # worker 有意没有 eval_truth path 参数或读取操作。
...
def evaluate_run(run_dir: str | Path) -> dict:
    """仅在坐标 hashes 完成后执行隔离 synthetic-truth evaluation。"""
    run_dir = Path(run_dir)
    manifest = verify_frozen_run_manifest(run_dir)
    results = {}
    for condition, spec in CONDITIONS.items():
...
def write_summary_tsv(run_dir: str | Path) -> Path:
    """为正式运行写出紧凑的非图形 plot-table artifact。"""
    run_dir = Path(run_dir)
