"""054 粗化：第四组最后一步，200kb 物理坐标 -> 原始 1Mb bin 的算术均值。
...
* partial bin 数由各 chr header 长度逐位推导，不硬断言 40 个。

输出 coords/200kb-to-1Mb/coarsened1Mb.{npz,3dg}、200kb 端点副本与
records/coarsening_manifest.json。1Mb 中间端点 coords/new-chain/1Mb.npz 保持不动。
"""
...
from bootstrap_054 import contact_model, data_io, round_runner  # noqa: E402
from round_paths_054 import FIT_ID, REUSED_AGGREGATES, ROOT, stage_bin  # noqa: E402

COARSE_BP = 1_000_000
OUT_DIR = RUN / "coords" / "200kb-to-1Mb"
MANIFEST = OUT_DIR / "coarsening_manifest.json"
FINE_AGGREGATE = REUSED_AGGREGATES[200_000][0]
COARSE_AGGREGATE = REUSED_AGGREGATES[1_000_000][0]
...
def main() -> int:
    ...
    fine = data_io.load_aggregate(FINE_AGGREGATE)
    ...
    # 起点 raw_y 自洽（端点由本链写出，必须能往返）
    if not np.allclose(contact_model.sphere_forward(raw_y), coordinates, atol=1e-10):
        raise RuntimeError("200kb endpoint raw_y does not map back to its coordinates")
    ...
    if n_loci_1mb != 2645 or expected_total != 13181:
        raise RuntimeError("frozen 1Mb target grid changed: loci=%d beads=%d" % (n_loci_1mb, expected_total))
    ...
    if total_beads != 26362 or int(complete.sum()) != 5256 or int(partial.sum()) != 34:
        raise RuntimeError("frozen bead accounting changed: total=%d five=%d partial=%d"
                           % (total_beads, int(complete.sum()), int(partial.sum())))

    OUT_DIR.mkdir(parents=True, exist_ok=True)
    # 200kb 端点副本（本链最终细层端点）；不改 coords/new-chain/1Mb.npz
    fine_3dg = OUT_DIR / "200kb.3dg"
    fine_npz = OUT_DIR / "200kb.npz"
    for src, dst in ((RUN / "coords" / FIT_ID / "200kb.3dg", fine_3dg),
                     (RUN / "coords" / FIT_ID / "200kb.npz", fine_npz)):
        if not src.is_file():
            raise RuntimeError("missing chain endpoint copy source: %s" % src)
        dst.write_bytes(src.read_bytes())

    # 用完整 1Mb grid 写出 .npz / .3dg（write_full_tracks 需要 header 层）
    ref_layer = data_io.load_aggregate(COARSE_AGGREGATE)
    ...
    manifest = {
        "schema": "p9016-round054-coarsening-v1",
        ...
        "intermediate_1mb_endpoint_preserved": str((RUN / "coords" / FIT_ID / "1Mb.npz").relative_to(ROOT)),
