"""全基因组数据层：20 条染色体、cis 与 inter contacts、40 条 tracks。

track 命名为 `c{1-based chromosome index:02d}{a|b}`，例如 `c01a`、`c20b`。
...
def track(ci, k):
    """返回 chromosome index `ci`（从 0 开始）和 copy `k`（取 0 或 1）的 track name。"""
...
def global_bin(ci, pos, counts):
    """返回拼接 chromosome bin 轴上的全局 bin index。"""
