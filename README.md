...
**Binary exceptions included on purpose:** exactly two figures —
`test_res/055-20260916_112451-max-contact-vs-baseline-review/plots/chr1_distance_matrices.png` and
`.../plots/same_cross_contrast_boxplot.png`.
...
**The current state of the work is:** `docs/CURRENT_BASELINE.md` +
`docs/current_baseline.json` for the registered baseline, and rounds **045 / 046 / 049** (fit and
evaluation implementations) with **051 / 055** (baseline readouts and the latest comparison) for the
most recent evidence. Where an old document and a current round disagree, the current round's frozen
`config.json` and readouts win.

## 7. Read order

1. `docs/GPT_PRO_REVIEW.md` — definitions, evidence map, limits, and the review prompt.
2. `docs/CURRENT_BASELINE.md`, `docs/current_baseline.json` — the registered baseline.
3. `docs/PROJECT_CONTEXT.md` — stage history and applicable scope (**historical context only**).
4. `docs/MEASURED_FACTS.md` — evidence log and protocol corrections.
5. `docs/legacy_workspace_AGENTS.md` — original project rules: scientific boundaries and reporting floor (Chinese, partly stale).
6. Code: `run.py`, then `pr/paths.py`, `pr/contact_model.py`, `pr/reconstruct.py`,
   `pr/reconstruction_evaluate.py`, `pr/continuation.py`.
7. Current chain: `test_res/049-.../source/max_contact_objective.py` (loss A/B/C),
   `test_res/045-.../source/*` (shared-capture objective and controller),
   `test_res/055-.../source/evaluate_review.py` (independent metric recomputation).
8. Readouts: `test_res/055-.../eval/{summary.json,per_chromosome.tsv,validation.json}`.
...
- **759 files, ~16.0 MB** of content (excluding `.git`).
- 754 files were copied from the workspace; of these 753 are byte-for-byte identical to their
  originals and 1 (`docs/current_baseline.json`) is a rebuilt review version. 5 files are added for
  this snapshot: `README.md`, `.gitignore`, `docs/GPT_PRO_REVIEW.md`, and the two renamed legacy
  copies `docs/legacy_workspace_README.md` / `docs/legacy_workspace_AGENTS.md` (the latter two are
  themselves byte-identical to the workspace `README.md` / `AGENTS.md`).
- Byte identity was verified against source SHA256 records after copying; 0 mismatches, 0 missing,
  0 unexpected files.
- **Dependency closure:** all 494 Python files parse; every import that is neither standard library
  nor a declared external package (`numpy`, `scipy`, `matplotlib`, `plotly`, `PIL`, `torch`,
  `threadpoolctl`) resolves inside the snapshot — 0 unresolved local imports.
...
- No file exceeds 1.2 MB; the only binaries are the two intentionally included PNG figures.
...
- Only static checks were performed (syntax/AST parse, import resolution, byte comparison, credential
  scan). **No training, fitting or evaluation code was executed.**
