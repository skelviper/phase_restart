"""无 phase 的 full-grid contact 聚合与连续 V1 目标函数。

本模块有意独立于 ``run.py`` 和 native FDG；其训练侧加载器只接受七列 SNP-free pairs schema。
"""
...
class TrackSpec:
    """与 SNP-free header order 明确绑定的一条输出轨迹。"""
...
class AggregatedContacts:
    """固定基因组分辨率下的 full-grid 聚合计数。

    ``pair_i``/``pair_j`` 枚举每个符合结构指标条件的 pair，而不只是输入中观测到的 pair。``diag_counts`` 是独立的同基因组 bin 计数层，按同一完整 locus grid 编索引。
    """
    ...
    def global_locus(self, chromosome_index: np.ndarray | int,
        ...
        """将 header-order 的 chromosome/bin 坐标映射到完整全局 grid。"""
    ...
    def budget(self) -> dict:
        """返回精确的 raw 或紧致的 synthetic-expected 计数核算。"""
...
def verify_frozen_snpfree(path: str | Path = SNPFREE) -> dict:
    """核验授权的 SNP-free 数据，不打开原始 phase 数据。"""
...
def _upper_pair_index(i: np.ndarray, j: np.ndarray, n_loci: int) -> np.ndarray:
    """完整 upper triangle 的扁平 row-major index，其中 ``i < j``。"""
...
def aggregate_from_arrays(chromosome_names: Sequence[str], chromosome_lengths: Sequence[int],
    ...
    """将全部 raw records 聚合到完整 header bin-pair grid。

    接受无 label 的数值数组，使小型 synthetic fixture 能与生产 loader 使用同一构造。既不创建 folds，也不接受 label 列。
    """
...
def synthetic_expected_clone(template: AggregatedContacts, counts: np.ndarray,
    ...
    """构建显式的 fractional expected-count 校准数据集。

    仅用于 synthetic noiseless cross-entropy 检查。它保留 template 的完整 header grid 和 track mapping，但不把 fractional expected mass 假装成 Poisson observations。因此 factorial constants 标记为不可用，且绝不用于 candidate selection。生产 SNP-free aggregation 仍为 ``raw_integer`` 且保持精确。
    """
...
def endpoint_counts_from_aggregates(template: AggregatedContacts, counts: np.ndarray,
    ...
    """从 aggregate counts 推导每个 full-grid locus 的双端 mass。"""
...
def synthetic_integer_clone(template: AggregatedContacts, counts: np.ndarray,
    ...
    """在现有 full-grid metadata 上构建显式整数 synthetic 数据集。

    不同于 :func:`synthetic_expected_clone`，这里表示 conditional multinomial integer draws。其 count-factorial constants 在数学上有定义并保留供 audit 使用，而 optimizer 仍省略与 candidate 无关的 conditional constants。
    """
    template.assert_consistent()
    if not exposure_mode:
...
def load_aggregate(path: str | Path = SNPFREE, bin_size: int = 1_000_000,
    ...
    """读取 SNP-free pairs 文件，并在不进行 fold split 的情况下聚合全部记录。"""
    path = str(path)
...
def load_frozen_p9016_aggregate(bin_size: int) -> AggregatedContacts:
    """读取唯一授权的真实输入，并断言其冻结的 raw budget。"""
...
def sphere_pullback(y: np.ndarray, gradient_x: np.ndarray) -> np.ndarray:
    """返回 :func:`sphere_forward` 的解析 Jacobian-vector product。"""
    y = np.asarray(y, dtype=np.float64)
...
def bounded_kernel(delta: np.ndarray, r0: float, epsilon: float = EPSILON,
    ...
    """返回有限 kernel，以及可选的相对于 ``delta`` 的导数。"""
...
def p_from_q(q: float) -> tuple[float, float]:
    """返回有界的 cis mixture 值及其相对于 q 的解析导数。"""
...
def q_from_p(p: float) -> float:
    """适用于有效初始 p 的 bounded-logistic 参数化逆变换。"""
...
class JointObjective:
    """解析的 V1 full-grid observation objective 与 polymer prior。"""
    ...
    def __init__(self, data: AggregatedContacts, weights: ObjectiveWeights | None = None,
        ...
        # 保留 optimizer 的精确 theta 对象；callback 只能在逐元素精确相等时复用该结果，
        # 不能使用 allclose 匹配。
    ...
    def rates_for_pair_indices(self, x: np.ndarray, p: float,
        ...
        """返回所选 full-grid pair rows 的 rates；供 audit/tests 使用。"""
        x = np.asarray(x, dtype=np.float64)
    ...
    def _diag_nll(self) -> float:
        """与 candidate 无关的同 bin profile；fractional 模式为 cross-entropy。"""
    ...
    def _count_nll_and_gradient(self, x: np.ndarray, p: float,
        ...
        # 第一遍：完整 eligible-grid 的归一化和观测 log-rate 项。
        ...
        gradient_p = 0.0
        # 第二遍：对同一完整 full-grid sums 求精确导数。
        for start in range(0, data.n_pairs, self.block_size):
            stop = min(start + self.block_size, data.n_pairs)
    ...
    def evaluate(self, theta: np.ndarray, need_gradient: bool = True):
        """返回 total、解析 gradient，以及可分别审计的 components。"""
        y, q = self.unpack(theta)
        x = sphere_forward(y)
        assert_inside_unit_ball(x)
    ...
    def count_components_for_coordinates(self, x: np.ndarray, p: float) -> dict:
        """仅针对固定 physical coordinates 评估无 label 的 count layer。"""
        x = np.asarray(x, dtype=np.float64)
        if x.shape != (2, self.data.n_loci, 3):
            raise ValueError("x must have shape (2, n_loci, 3)")
        assert_inside_unit_ball(x)
    ...
    def cached_value_and_components(self, theta: np.ndarray):
        """当 ``theta`` 未改变时返回最新的精确 value/components。

        cache 有意保留原始 float64 theta 对象，而不是四舍五入后的日志表示。只有通过逐元素精确相等检查的调用者才能收到该缓存，因此更新后的 iterate 不会被误认为是产生 analytic gradient 的那个 iterate。
        """
    ...
    def value_and_grad(self, theta: np.ndarray) -> tuple[float, np.ndarray]:
        ...
        # 不复制 theta：SciPy callback 返回精确的当前 iterate，随后使用精确匹配，
        # 而不是基于容差的匹配。
...
def tracks_from_coordinates(data: AggregatedContacts, x: np.ndarray) -> dict[str, dict[int, np.ndarray]]:
    """返回全部 40 条、按 header order 排列的 tracks，并表示每个 full-grid bin。"""
    x = np.asarray(x, dtype=np.float64)
...
def write_full_tracks(path: str | Path, data: AggregatedContacts, x: np.ndarray) -> None:
    """按明确的 SNP-free-header track order 写出每个 copy 和 full-grid bin。"""
