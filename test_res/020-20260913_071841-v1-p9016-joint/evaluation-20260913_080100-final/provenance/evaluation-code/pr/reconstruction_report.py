...
def write_fragment_stripes(path: str | os.PathLike[str], table_path: str | os.PathLike[str],
    ...
    """Render the 20 Mb R3 fragment labels with missing/tied states explicitly grey."""
    from . import figs
    import matplotlib.pyplot as plt
    ...
    ax.set_xlabel("20 Mb fragment start on OFF=3 Mb metric grid (Mb)")
    ...
    fig.text(0.5, 0.005, "grey = missing, insufficient, or geometry-tied fragment; these states are not scored",
             ha="center", va="bottom", fontsize=7)
    fig.subplots_adjust(bottom=0.17, left=0.12, right=0.92, top=0.98)
    fig.savefig(target, dpi=300)
    plt.close(fig)
    ...
    return {"figure": str(target), "table": str(table_target), "n_chromosomes": len(chromosome_names),
            "n_fragments_max": n_columns, "missing_and_tied_color": "grey"}
