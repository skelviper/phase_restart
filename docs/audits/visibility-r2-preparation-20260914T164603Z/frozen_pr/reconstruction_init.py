"""Reconstruction V1 的冻结、无 phase 坐标初始化。
...
def expand_tracks_to_full_grid(source_tracks, names, header_lengths, bin_size, candidate):
    """将内存中的 source tracks 扩展为完整的 copy-first full grid。
    ...
    for chromosome, length in enumerate(lengths):
        ...
        for copy in (0, 1):
            ...
            # Consensus 只有一条 source trajectory，并复制到两个 candidate copies。
...
def initialize_from_tracks(source_tracks, names, header_lengths, bin_size, candidate, seed,
    ...
    """从内存中的无 phase tracks 创建归一化的 V1 初始化。

    这个通用函数从不读取路径，适用于 synthetic tests 和 adapters。真实 P9016 启动必须调用 :func:`initialize_approved_candidate`，由它核验冻结 source。
    """
...
def initialize_approved_candidate(candidate, names, header_lengths, bin_size):
    """不做优化地初始化两个冻结的 P9016 blind candidates 之一。

    consensus candidate 使用 seed 1103 和平滑的非零 half-difference；random 使用 seed 2207，并保留其两条 native source tracks。两种选择都不代表 independent fit 或 biological replicate。
    """
    if candidate not in APPROVED_SOURCES:
...
def warm_start_from_layer(coords, positions, chromosome_index, names, header_lengths, bin_size,
                          candidate_base_seed):
    """将上一层的 40-track layer 插值到更细的完整 full grid。

    保留传入的 all-cell 坐标系和尺度。只扰动新引入/重复的 loci，然后将达到或超出单位球的坐标径向裁剪到 ``1 - 1e-6``。
    """
    started = time.monotonic()
    layout = full_grid_layout(names, header_lengths, bin_size)
