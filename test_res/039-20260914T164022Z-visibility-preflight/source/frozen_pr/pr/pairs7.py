...
`run.py prepare` 流式读取 `data/P9016.pairs.gz`，写出供所有 training-side 步骤使用的七列文件。只要 `#columns` 行提到 phase 字段，loader 就会拒绝该文件，避免误读带 phase 的输入。
"""
import gzip
import os
...
def canonical_cis(pos1, pos2):
    ...
    携带逐端 metadata 的调用者必须在该标记为 true 时同步交换 metadata。将规则集中在此处，可以防止位置规范化时把端点 label 与坐标悄悄拆开。
    """
...
def iter_records(path):
    ...
    仅验证 header 不够：附加的第八列或被截断的数据行，否则可能通过位置数组索引进入 training。该 reader 有意保持严格，供 single- 和 genome-wide loaders 共同使用。
    """
    assert_snpfree(path)
    with _open(path) as f:
