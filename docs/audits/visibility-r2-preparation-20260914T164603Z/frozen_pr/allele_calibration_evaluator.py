"""Per-chromosome evaluation for the synthetic R2 allele calibration.

This module intentionally does not call the historical ``allele_calibration.evaluate_run``
implementation.  Candidate rows are globally swapped independently for each chromosome,
while truth reference columns remain fixed as mat (ref1) and pat (ref2).  Truth files are
opened only after the fit manifest and every candidate coordinate hash have passed.
"""
...
class EvaluationError(RuntimeError):
    """Raised when an evaluation-side hash or metric contract is invalid."""
...
def _fixed_abcd(raw: Mapping[str, float | None], orientation: str) -> tuple[float | None, ...] | None:
    ...
    if orientation == "swapped":
        # Only the candidate rows move. Truth reference columns remain mat/pat.
...
def _shape_errors(candidate_a: np.ndarray, candidate_b: np.ndarray,
    ...
    if same_shape:
        # N has no identifiable truth copy. Both candidate rows are scored against
        # the same common truth matrix and no identity is inferred.
        assigned_a, assigned_b = truth_a, truth_b
    elif orientation == "direct":
        assigned_a, assigned_b = truth_a, truth_b
    elif orientation == "swapped":
        # Candidate rows, not truth rows, are swapped for the cross orientation.
...
def chromosome_metrics(template: Any, candidate: np.ndarray, truth: np.ndarray,
    ...
    """Compute one chromosome's raw/derived/fixed-reference metrics."""
    ...
    if tie:
        # Raw four-rho is retained, but no named reference orientation is chosen.
...
def evaluate_run(run_dir: str | Path, candidate_manifest_path: str | Path | None = None) -> dict[str, Any]:
    """Evaluate locked endpoints with independent per-chromosome swaps."""
    ...
    # This validates all 20 candidate file hashes/domain before the first truth path is opened.
