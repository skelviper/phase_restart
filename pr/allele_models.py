"""Post-020 allele 模型变体及隔离的坐标参数化。

冻结的 V1 objective 仍是默认实现。本模块只增加五个预注册变体的描述、C3 使用的统一 exposure 视图，以及两个 C2 坐标域的轻量 objective 子类。

这里不导入 native FDG、带 phase 的输入或 reference artifact。
"""
...
def data_for_model(data: AggregatedContacts, model_id: str) -> AggregatedContacts:
    """返回某个变体的数据视图，不改变 counts 或 eligibility。

    C3 只替换 exposure vector 及其显式 mode label。endpoint audit、完整的 eligible pair grid、diagonal nuisance counts 和所有 conservation fields 在适用处逐字节保留。
    """
...
def identity_interior_forward(y: np.ndarray) -> np.ndarray:
    """将 R^3 映射到开单位球，半径 0.90 以内保持 identity。

    外层径向映射在 0.90 处为 C2；对有限的普通浮点输入，它趋近但不会有意越过半径一。
    """
    values = _as_xyz(y, "y")
...
def identity_interior_inverse(x: np.ndarray) -> np.ndarray:
    """在开单位球上求 :func:`identity_interior_forward` 的逆。"""
    values = _as_xyz(x, "x")
    radius = np.linalg.norm(values, axis=-1)
...
def identity_interior_pullback(y: np.ndarray, gradient_x: np.ndarray) -> np.ndarray:
    """返回 identity-interior map 的转置 Jacobian 乘积。"""
    values = _as_xyz(y, "y")
...
class _MappedJointObjective(JointObjective):
    ...
    def evaluate(self, theta: np.ndarray, need_gradient: bool = True):
        """在显式 physical map 后评估未改变的 V1 terms。"""
        raw, q = self.unpack(theta)
...
class IdentityInteriorJointObjective(_MappedJointObjective):
    """C2-map objective：使用平滑 identity-interior map 的相同 V1 项。"""
...
class DirectPhysicalJointObjective(_MappedJointObjective):
    """C2-free objective：原始优化器变量是有限的 physical x。"""
...
def raw_coordinates_from_physical(objective: JointObjective,
    ...
    """将双拷贝 physical start 转换为 objective 的原始变量。"""
...
def physical_coordinates_from_raw(objective: JointObjective,
    ...
    """将一个 objective 的原始变量转换为 physical coordinates。"""
...
def validate_physical_for_model(model_id: str, coordinates: np.ndarray) -> None:
    """对已注册变体应用精确的 physical-domain validator。"""
    values = np.asarray(coordinates, dtype=np.float64)
    spec = model_spec(model_id)
