"""Guarded final evaluation and delivery orchestration for Reconstruction V1.

This module is the evaluator-side entrance used after a completed training run.
It deliberately keeps the boundary between phase-free training and evaluator
inputs explicit:

* selection/provenance/candidate coordinate checks happen before any evaluator
  loader is called;
* coordinate paths are registered in a fresh :class:`EvalGate` before the gate
  is armed for phase/reference access;
* contacts, labels, and reference are loaded through injected callbacks, making
  the complete sequence testable with a synthetic fixture;
* selection is never recomputed from reference metrics.  ``selected_id`` is the
  training-time choice carried by ``selection.json``.

The default callbacks are the production P9016 callbacks.  Tests and future
formal wrappers may inject callbacks with the same signatures as
``reconstruction_report.guarded_final_evaluation_inputs``.
"""
...
class EvaluationError(RuntimeError):
    """Raised when final-evaluation orchestration or delivery is invalid."""


def _safe(value: Any) -> Any:
    """Convert NumPy/path values to strict JSON-compatible primitives."""
...
def _resolve_asset_path(value: Any, base: Path) -> Path:
    """Resolve a path stored by a formal run without opening its payload."""
    ...
    # Keep a deterministic path for the later guard to report as unavailable.
...
def baseline_specs_from_selection(
    ...
    """Build explicit baseline source specs from selection metadata.

    ``pr.reconstruct`` stores the 014 source gate and coordinate references under
    ``source_gate``/``source_coordinate``.  This helper only reads selection
    metadata; source bytes are opened later by ``verify_baseline_source``.
    """
...
def _default_baseline_loader(_grid: report.FullGrid) -> Callable[[str, str], Any]:
    def load(_tag: str, path: str) -> Any:
        # The source gate has already checked its digest and track inventory.
        # The strict coordinate parser keeps malformed source rows from entering
...
def alignment_verifier(contacts: Any, raw_path: str) -> dict[str, Any]:
    """Compare every raw record to SNP-free contacts numerically and in order.

    Only chromosome and coordinate columns are inspected here.  Phase columns
    remain untouched until ``refeval.load_labels_two`` is called after the gate
    is armed.  For cis records, endpoint order is canonicalised numerically and
    the swap count is retained as audit evidence.
    """
...
def _metric_bins(length_bp: int) -> int:
    # The full training grid starts at zero.  OFF=3 Mb is a separate metric
    # grid and includes only starts strictly below the chromosome header length.
...
def _termination_flags(fit: Mapping[str, Any], audit: Mapping[str, Any] | None = None) -> dict[str, Any]:
    """Normalize iteration/function limit evidence without rewriting legacy fields."""
...
def _stage_termination_audit(audit: Any, stage: Any) -> Mapping[str, Any] | None:
    """Select one stage's record from the possible audit JSON shapes."""
...
def _candidate_termination_audit(
    ...
    """Load an optional independent termination audit after training validation."""
...
def _solver_layer_metadata(
    ...
    """Read frozen stage summaries without carrying the large optimizer history."""
    result = []
...
def _candidate_metadata(verified: report.VerifiedSelection) -> dict[str, dict[str, Any]]:
    ...
    for candidate_id in verified.document.get("preregistered_candidate_ids", []):
        ...
        item: dict[str, Any] = {
            ...
            # Preserve the runner's legacy field verbatim; use the explicit
            # derived flags below for human-facing budget interpretation.
...
def _comparison(
    ...
    # Use stable metric-specific seeds so reports are reproducible across processes.
...
def _candidate_summary(
    ...
    r1 = [row["R1"].get("accuracy") for row in rows.values()]
...
def _fixed_baseline_comparison(
    ...
    """Keep the pre-existing oracle-vs-random ceiling comparison explicit."""
...
def _augment_metric_row(row: dict[str, Any]) -> dict[str, Any]:
    """Expose one stable candidate-minus-random sign at every readout."""
    r1 = row["R1"]
    ...
    for candidate_id in verified.document.get("preregistered_candidate_ids", []):
        ...
        for chromosome in per_chromosome:
            ...
            rows.append({
                ...
                "selected_by_training_count_nll": candidate_id == verified.selected_id,
                "count_nll_per_record": candidate.count_nll_per_record,
                "R1_accuracy": row["R1"].get("accuracy"),
                ...
                "R3_status_counts_json": _json_cell(_r3_status_counts(row["R3"])),
            })
...
def _render_readme(
    ...
    *,
    ...
    lines = [
        "# P9016 单细胞最终评价与交付",
        "",
        "**状态：** `evaluation_complete`。本目录由训练完成后的新 evaluation 子目录生成，不覆盖 formal training 文件。",
        "",
        "## 训练选择与冻结边界",
        "",
        "- 生物学样本数为 1；冻结范围是全部 1,703,888 records = 1,135,454 cis + 568,434 inter。",
        "- SNP-free 训练输入 SHA256：`%s`；训练网格为从 0 开始的 1 Mb full grid，%d loci / %d physical beads，所有 40 轨位于同一无量纲 R=1 核球内。R=1 没有微米尺度标定。" % (
            verified.document["cohort"]["snpfree_sha256"], verified.grid.n_loci, verified.grid.n_physical_beads),
        "- 候选选择严格沿用 training-time `%s` / `%s` 和预注册顺序；本评价未按 reference 重新选择。冻结 selected_id：`%s`。" % (
            verified.document["objective_contract"]["criterion"],
            verified.document["objective_contract"]["direction"], selected_id),
        "- 已重新核验 protocol/config/code/data 冻结哈希、training-code snapshot 每个成员、protocol manifest 每个成员、完整 attempts 和每个候选坐标 SHA。code snapshot 不依赖 workspace 当前源码。",
        "",
        "### 候选终态",
        "",
        "- `budget_exhausted` 是旧 runner 字段；最终预算解释优先使用独立 `termination_audit`，否则由 solver 的 `nit/maxiter` 与 `nfev/maxfun` 推导。`not_converged` 且达到迭代上限应直写“迭代预算达到、尚未收敛”，不能解读为预算未到。",
    ]
    for candidate_id in verified.document.get("preregistered_candidate_ids", []):
        item = metadata[candidate_id]
        if item["terminal_status"] == "failed":
            lines.append("- `%s`：terminal=`failed`；明确失败原因：%s；无坐标、无可选 NLL。" %
        ...
        lines.append(
            "- `%s`：terminal=`%s`；count_nll_per_record=`%s`；p=`%s`（nuisance coefficient，不是 same-copy 百分比）；prior(bond,repulsion,bend,p_prior)=(%s,%s,%s,%s)；budget_reached=`%s`（iteration_limit_reached=`%s`，function_limit_reached=`%s`，原始 budget_exhausted=`%s`）；training_selected=`%s`。" % (
        ...
        if item.get("solver_layers"):
            lines.append("  solver layer终态：" + "; ".join(
    ...
    lines.extend([
        ...
        "## 输入审计与 gate",
        "",
        "- 读取 phase/reference 前，先核 raw pairs SHA、reference SHA（严格 P9016 要求公布的 017 digest `%s`）和已 hash 的 017 provenance 声明；SNP-free contacts 由冻结 data snapshot 加载。" % P9016_REFERENCE_3DG_SHA256,
        "- alignment 是逐 record 的数值 chromosome/coordinate 比较；cis 端点按数值 canonicalization，并将端点交换计入 alignment 证据。labels loader 返回两端 a1/a2 全长 M，并同步交换 phase 端点；随后才读取 reference。",
        "- 本目录保存 `record_alignment` 全量摘要、输入版本、gate entries 与 evaluator code snapshot；这些不写回 training frozen manifest。",
        "",
        "## 指标口径",
        "",
        "- R1 只评估同 chromosome、same-copy 已定相、非同 bin、OFF=3 Mb metrics grid 记录。每个 candidate、fixed random、fully phased S0 oracle-fit 和 reference ceiling 在各自 candidate/random/oracle/reference 全 finite 的共同 record mask 上计算；tie、missing、out-grid 和 denominator 均单列。它不是 held-out 泛化，也不是数学绝对上界。inter contacts 不用于 haplotype accuracy。",
        "- R2 使用 candidate/random/reference 六轨共同 finite 非对角 pair mask，并按每条染色体 geometry gauge 决定 copy pairing。",
        "- R3 使用共同可解析的 20 Mb fragments；`labelled`、`local_geometry_tie`、`insufficient_common_finite_pairs`、缺失状态和断 run 都保留。majority 是正向 baseline，不把 low wall 数单独命名为 L2。",
        "- 所有 nonfailed 预注册候选均与固定 random baseline 对比，方向统一为 `delta_candidate_minus_random`。wins 与 paired chromosome bootstrap CI 只表示一个细胞内的技术/结构变化，不是 biological replicate uncertainty；不使用不同分母的旧 macro mean 相减。",
        "- consensus 单轨迹的 two-copy R1/R2/R3 contrast 为 n/a，但其单结构 baseline 保留。所有 negative/不可用结果照实保留，不因 L2 未证而省略候选结构。",
        "",
        "## L2 边界",
        "",
        "- L2 结论：%s" % l2_conclusion,
        "- R3 是 L2 的主要读出；低 wall 或局部 majority 本身不构成整条染色体 copy identity 恢复证据。",
        "",
        "## 交付文件",
        "",
        "- selected 原始 R1 unit-ball 坐标下载：`%s`；track map：`%s`。" %
        (delivery["selected_coordinates"]["path"], delivery["track_map"]["path"]),
        "- selected 静态 3D：`%s`；self-contained canvas HTML：`%s`。HTML 的旋转/缩放/chr/copy 开关、hover 和 raw export 只改变显示投影，不改变 raw 坐标。" %
        (delivery["selected_figure"]["figure"], delivery["selected_html"]["html"]),
        "- 全候选坐标副本、metrics.json、per-chromosome TSV、summary、R3 stripes、track coverage、candidate gallery 和 coolwarm_r distance-map 均在本 evaluation 子目录中。gallery 各 panel 独立 center/unit-RMS，仅作结构诊断，不声称共享 frame 或绝对微米尺度。",
        "- 本目录不打包真实 phase-bearing pairs 或原始 reference payload；只保存其版本哈希、alignment 证据和 evaluation code snapshot。",
        "",
        "## 汇总",
        "",
        "```json",
...
def run_final_evaluation(
    ...
    *,
    ...
    l2_conclusion: str = "L2 未证：最终结论必须结合全 chromosome R3 一致性、共同分母和 paired CI；本入口不把低 wall 或局部 majority 自动升级为 L2。",
    synthetic_fixture: bool = False,
) -> dict[str, Any]:
    """Run the complete guarded evaluator and write a fresh delivery directory.

    Required callback signatures, when injected, are:

    ``candidate_loader(path)`` -> coordinate mapping;
    ``baseline_loader(tag, path)`` -> coordinate mapping;
    ``contacts_loader()`` -> ``genome.load_all``-compatible mapping;
    ``alignment_verifier(contacts, raw_path)`` -> ``{"aligned": True, ...}``;
    ``labels_loader(contacts, raw_path)`` -> ``(a1, a2)`` arrays;
    ``reference_loader(path)`` -> reference coordinate mapping.

    With no injected callbacks, the function uses the production SNP-free,
    ``refeval.load_labels_two`` and ``ref3dg.load_reference`` loaders.  It does
    not start training or select a candidate from reference metrics.
    """
    selection_file = Path(selection_path).resolve()
    # Pure preflight gives us the verified grid/data snapshot for constructing
    # default closures and baseline specs.  It cannot read labels/reference.
    ...
    try:
        # Audit the published P9016 reference digest before any evaluator loader.
        # This hashes bytes only; it does not parse phase/reference payloads.
    ...
    def guarded_labels(contacts: Any, raw_path: str) -> Any:
        # Baseline paths are explicit in specs; no baseline payload has been
        # opened before this point, but every source file was hash-verified by
        # the guard immediately before its loader was called.
        expected = {
    ...
    try:
        ...
        # The output path remains a fresh, inspectable failure directory, but no
        # evaluator artifact is claimed complete when the guard aborts.
        _write_json(root / "failure.json", {
    ...
    # Copy every nonfailed candidate payload into the new evaluation directory;
    # failed candidates intentionally receive no coordinate path.
...
# Short alias for callers that want a command-like API.
