...
WORK = "scratch/basin4"; os.makedirs(WORK, exist_ok=True)
...
FS = [0.0, 0.2, 0.35, 0.5]; ROUNDS = 8; ALPHA = 1.0
    ...
    for f in FS:
        ...
        for k in range(ROUNDS + 1):
            ...
            if mode == "hard":
                ...
                pA = 1.0 / (1.0 + np.exp(-ALPHA * np.clip(sc, -30, 30)))
                asg = (rng.random(N) < pA).astype(int)
