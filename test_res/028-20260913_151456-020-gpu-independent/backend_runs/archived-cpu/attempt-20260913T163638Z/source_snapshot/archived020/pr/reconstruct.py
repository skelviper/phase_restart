"""Formal, phase-free Reconstruction V1 production runner.

This module prepares and executes only the frozen all-contact coordinate training
path.  It never imports phase labels, reference coordinates, oracle coordinate
loaders, or evaluator-side metric functions.  Selection is strictly the frozen
label-free count objective after final coordinate serialization and readback.
"""
...
class ReconstructionError(RuntimeError):
    """Raised when a frozen formal training invariant is violated."""


@dataclass(frozen=True)
class StageSpec:
    """One pre-registered multiresolution optimization layer."""
...
@dataclass(frozen=True)
class CandidateSpec:
    """A pre-registered, label-free initialization family."""
...
@dataclass(frozen=True)
class RunContext:
    """All immutable inputs needed by the generic runner and its test fixtures."""
...
@dataclass(frozen=True)
class RunnerHooks:
    """Injectable phase-free collaborators used by synthetic tests only."""
...
def _json_ready(value: Any) -> Any:
    """Convert NumPy values while rejecting non-finite formal metadata."""
...
def _run_relative_path(path_text: Any, label: str) -> str:
    """Validate a manifest path without ever consulting its historical source."""
...
def _frozen_source_assets() -> tuple[dict[str, dict[str, Any]], tuple[dict[str, Any], ...]]:
    """Read only the 014 gate metadata; never open oracle coordinates."""
    ...
    for tag, role in (("consensus", "blind_baseline"), ("random", "blind_baseline"),
        ...
        # Coordinate paths/digests are copied from the source gate as provenance only.
        # In particular, this does not open or hash the oracle coordinate payload.
...
def production_context() -> RunContext:
    """Construct the only real-data context allowed by the frozen V1 runner."""
...
def _write_static_run_files(outdir: Path, context: RunContext, config: Mapping[str, Any]) -> dict[str, Any]:
    ...
    readme = """# Reconstruction V1 Formal Training\n\nThis directory is a phase-free coordinate-training record. It uses the frozen SNP-free P9016 input, the pre-registered consensus_joint and random_joint initializations, and the 5 Mb -> 2 Mb -> 1 Mb schedule. Dynamic execution state is under `run_status/`; immutable training configuration and snapshots are under `config.json` and `provenance/`. No phase labels, reference coordinates, or oracle coordinate payloads are opened by the training runner.\n"""
...
def _attempt_journal_event(attempt: Mapping[str, Any]) -> str:
    """Choose one stable journal event for each final attempt record."""
...
def _solver_status(result: Any, stage: StageSpec) -> tuple[str, bool, str]:
    nfev = int(getattr(result, "nfev", getattr(result, "actual_nfev", 0)))
    message = str(getattr(result, "message", ""))
    budget_exhausted = nfev >= stage.maxfun or "TOTAL NO. of f" in message.upper()
    success = bool(getattr(result, "success", False)) and not budget_exhausted
    if success:
        return "converged", False, "solver_reported_success"
    return "not_converged", budget_exhausted, "budget_exhausted" if budget_exhausted else "solver_reported_nonconvergence"
...
def select_candidate(candidates: list[Mapping[str, Any]], preregistered_order: tuple[str, ...] | list[str]) -> str:
    """Select only by final count NLL, with the frozen per-record tie rule."""
    ...
    for raw in candidates:
        ...
        if raw["optimization"]["terminal_status"] == "failed":
            ...
            record.update({
                ...
                # This map must contain exactly report.COUNT_MODEL_FIELDS.
...
def verify_completed_training(outdir: str | os.PathLike[str], *, strict_p9016: bool = True) -> Any:
    """Run only pure provenance/inventory guards; never invoke evaluator loaders."""
...
def run_training(outdir: str | os.PathLike[str], context: RunContext, *, hooks: RunnerHooks | None = None,
    ...
    """Execute a frozen context, intended for production and synthetic runner tests.

    The real entry point is :func:`run_production`; injected hooks exist solely so
    tests can exercise journaling and provenance without optimizing P9016.
    """
    ...
    # Persist one final, deterministic journal row per attempt after candidate-level
    # rescore has established the single terminal flag for each candidate.
    ...
    # This guard reads only snapshots, data hashes, track map, selection metadata, and final
    # candidate coordinates. It does not receive labels/reference/oracle loader functions.
...
def run_production(outdir: str | os.PathLike[str], *, workers: int = 1, threads: int = 1) -> dict[str, Any]:
    """Run the authorized P9016 V1 training path when the parent releases it."""
