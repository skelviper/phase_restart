...
class BackendAdapter:
    def __init__(self, kind: str, device: str, tile_rows: int):
        if kind not in ("torch", "archived_cpu"):
    ...
    def setup_timing_name(self) -> str:
        return "upload_seconds" if self.kind == "torch" else "archived_constructor_seconds"
    ...
    for stage_index in range(start_stage, stop_stage + 1):
        ...
        stage_row = {
            ...
            "timing": {
                ...
                adapter.setup_timing_name(): setup_seconds if adapter.kind == "torch" else 0.0,
...
def main() -> None:
    ...
    parser.add_argument("--backend", choices=("torch", "archived_cpu"), default="torch")
