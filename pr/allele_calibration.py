"""准备并评估 post-020 合成等位基因校准。

prepare 阶段创建四个固定的整数计数夹具，以及每个夹具的五种模型视图。它绝不调用优化器。面向 worker 的 artifact 只包含无标签计数、拟合暴露量和一份共享的独立物理起点；隔离的合成真值单独写出，不列在 worker manifest 中。

evaluation 阶段在打开任何真值文件前核验候选文件哈希。它接受包含 ``coordinates``/``coords`` 的 NPZ，或 :func:`pr.paired_run.write_coordinates` 写出的文本格式。
"""
...
class CalibrationError(RuntimeError):
    """冻结的 prepare/evaluation 契约无法核验时抛出。"""
...
def _save_counts_snapshot(path: Path, template: Any, counts: np.ndarray, diag_counts: np.ndarray,
    ...
    """保存紧凑的原始计数快照供审计；不保存 exposure 或 truth。"""
    np.savez_compressed(
        path,
...
def _write_readme(run_dir: Path, summary: Mapping[str, Any]) -> None:
    lines = [
        "# 026 R2 合成 等位基因校准准备",
        "",
        "状态：`prepared_no_optimizer`。本目录只完成 N1/N2/P1/P2 的合成数据、独立起点、隔离真值和 4x5 运行计划；没有调用优化器，没有运行工作进程拟合，没有 R1/R3。",
        "",
        "## 冻结设计",
        "",
        "- 从 0 开始的 1 Mb 完整 `header`（文件头）网格：20 条染色体、40 条轨迹、2645 个位点、5290 个物理珠子。所有符合条件的非对角距离对都进入速率/计数网格；同区间 diag 是独立的多项式干扰项。",
        "- 生成核：V1，`epsilon=1e-6`，`r0=2*l0`，`p_gen=0.8`。每个夹具的 diag/cis-offdiag/inter 总数固定为 438774/696680/568434。",
        "- N1/N2 是内部形状相同但中心分离的负对照夹具；P1/P2 是两个内部形状不同的正对照夹具。N1/P1 使用 `ones` 暴露量；N2/P2 使用 sigma=.4、无丢弃、均值为一的暴露量。",
        "- C0/C1/C2-map/C2-free 的拟合暴露量都从合成观测端点重新计算为 `sqrt(endpoint+10)/mean`；C3 只使用 `ones`。生成暴露量不传给拟合。",
        "- 每个夹具的五个变体共用同一个独立物理 x0；x0 来自独立 `generate_truth(..., same_shape=False)`，再做一次全局居中/缩放到最大半径 .8，`p0=.75`。",
        "- 固定后续预算：`maxiter=80, maxfun=270, maxls=20, ftol=1e-10, gtol=1e-6`，每 20 步保存检查点；后续最多 4 个工作进程，BLAS/OMP=1。",
        "",
        "## 准备门控",
        "",
        "prepare 的 20 个 B 配对计划均为 `fit_not_run=true`；B 的 `data_for_model` 视图和每个层的 counts/endpoint/exposure 均已做往返校验。夹具门控失败时保留原 seed/artifact，不替换 seed。",
        "",
        "| 夹具 | 真值门控 | 计数零值/非零值（diag/cis/inter） | 共享 x0 最大半径 |",
        "|---|---|---|---:|",
    ]
    for fixture_id in FIXTURES:
    ...
    lines.extend([
        "",
        "当前结果：`planned_fit_count=20`，`actual_fit_count=0`，`optimizer_started=false`。N/P 真值距离 rho 只记录，不用于选择 seed 或模型。",
        "",
        "## 工作进程侧产物边界",
        "",
        "- [`work/prepared_manifest.json`](work/prepared_manifest.json) 只列无标签整数层、共享起点、模型 ID、候选输出位置和冻结的 FitConfig；不包含真值路径。",
        "- [`results/prepare_summary.json`](results/prepare_summary.json) 记录计数/网格/校验/计划和仅评价工件索引。",
        "- [`provenance/truth_manifest.json`](provenance/truth_manifest.json) 属于评价器侧；候选文件哈希必须全部通过后才能打开其中的真值。",
        "- [`provenance/source_manifest.json`](provenance/source_manifest.json) 记录文件头/代码哈希；没有读取真实 `reference`、`phase`、生产坐标或真实接触作为观测。",
        "",
        "## 后续评价适配器",
        "",
        "`python -m pr.allele_calibration evaluate --run-dir <this-run> --candidate-manifest results/candidate_manifest.json` 会先检查 20 个候选文件哈希、完整网格形状、有限值和模型物理域，再加载真值。它按夹具做一次整体候选互换，不做局部染色体修复；N 的真值距离矩阵使用两个拷贝的均值，仅 N 条件的拷贝差异 RMS 只作诊断。",
        "",
        "主要产物：`config.json`、`work/prepared_manifest.json`、4 个计数快照、20 个变体层、4 个共享起点、4 个隔离真值、`results/prepare_summary.json`。",
        "",
    ])
...
def prepare_run(run_dir: str | Path) -> dict[str, Any]:
    """准备全部 synthetic artifacts，并只运行非 optimizer 的 validation gates。"""
    ...
    for fixture_id, fixture in FIXTURES.items():
        # truth 和 initialization 使用分开的冻结 seed streams。initialization 独立生成，
        # 然后全局居中/缩放；它绝不读取 truth。
        ...
        # Exercise B 的统一 model/data contract，不调用 run_one_fit。
...
def load_prepared_data(run_dir: str | Path, fixture_id: str, model_id: str) -> tuple[Any, PairedStart, dict[str, Any]]:
    """加载一份面向 worker 的 layer/start pair，并核验其 prepared hashes。"""
    run_dir = Path(run_dir).resolve()
...
def _validate_candidate_manifest(run_dir: Path, manifest: Mapping[str, Any], template: Any) -> list[dict[str, Any]]:
    ...
    seen = set()
    validated = []
    # truth loading 前有意完成 hash 和基本 candidate validation。
...
def evaluate_run(run_dir: str | Path, candidate_manifest_path: str | Path | None = None) -> dict[str, Any]:
    """哈希核验完成后评估 20 个 candidates，再加载隔离的 truth。"""
    run_dir = Path(run_dir).resolve()
    ...
    # 上面没有打开 truth path。只有每个 candidate file 的 hash 和 domain gate 都通过后：
...
def prepare_and_write(run_dir: str | Path) -> dict[str, Any]:
    """无 optimizer preparation 阶段的 CLI wrapper。"""
