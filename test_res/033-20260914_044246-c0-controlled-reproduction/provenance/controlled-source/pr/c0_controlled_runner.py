"""Controlled C0 reproduction runner for the corrected post-020 design.

This module is training-side only.  It reads the frozen seven-column SNP-free
input and the two blind 014 coordinate sources, runs only C0 in this phase,
and never opens phase-bearing payloads, reference coordinates, evaluation
outputs, or oracle coordinate data.  The other four variants remain in the
frozen 30-attempt plan but are intentionally blocked until the parent releases
them.
"""
...
class ControlledRunnerError(RuntimeError):
    """Raised when the corrected C0 contract cannot be honored."""
...
def _build_common_starts(context: reconstruct.RunContext,
    ...
    # The same candidate's physical start is passed byte-for-byte to every variant.
...
def _c2_domain_checks(data: contact_model.AggregatedContacts, root: Path) -> dict[str, Any]:
    ...
    # Use the actual C2-map objective on a real phase-free layer and a raw point
    # outside the identity interior to exercise the adapter's pullback.
    c2_data = data
    ...
    # C2-free must preserve a finite physical point at radius > 1 through pack,
    # evaluate, and the model-specific writer; no sphere inverse/clip is allowed.
...
def run_preflight(context: reconstruct.RunContext, root: Path) -> dict[str, Any]:
    ...
    for candidate in CANDIDATES:
        for stage in STAGES:
            ...
            coords = _state_array(state)
            # The full common-start chain above is the layer-interface fixture.
            # Reuse its actual coordinates for all objective checks.
...
def _compare_checkpoint(old_path: Path, new_path: Path, old_history: Any,
    ...
    try:
        ...
        # Elapsed time is intentionally excluded; the added gradient field is
        # retained by the new runner but absent from the 020 record.
...
def build_c0_gate(root: Path, context: reconstruct.RunContext,
    ...
    for candidate in CANDIDATES:
        ...
        for stage in STAGES:
            ...
            # Gradient norm is new bookkeeping; elapsed is deliberately absent.
...
def _write_report(root: Path, gate: Mapping[str, Any], terminal: Mapping[str, Any],
    ...
    lines = [
        ...
        "- 范围: 仅 phase-free、reference-free 的 C0 训练；不含 R1/R2/R3 评价。",
        "- 当前放行: `consensus_joint` 与 `random_joint`，每个完整执行 `5m -> 2m -> 1m`。",
        "- 其余变体: `C1`, `C2-map`, `C2-free`, `C3` 的 24 个 stage attempt 已冻结但阻塞，等待父侧验收。",
        "",
        "## 冻结合同",
        "",
        "- 输入: `inputs/P9016.snpfree.pairs.gz`，SHA256 `%s`，全量 1,703,888 records。" % EXPECTED_INPUT_SHA256,
        "- 网格: origin 0；最终 2,645 loci、5,290 beads、40 tracks；所有 bin 保留 zero/terminal bins。",
        "- 优化预算: `5m 300/930 -> 2m 200/630 -> 1m 240/750`，`maxls=20`、`ftol=1e-10`、`gtol=1e-6`，每 10 个 accepted checkpoint。",
        "- 首层 `p=0.75`，后续层携带 raw `q`；选择为全量 label-free `count_nll_per_record` 最小化，tie tolerance `1e-9`，按预注册顺序破平局，不含 priors、包含 candidate-invariant diagonal 层。",
        "",
        "## 预检",
        "",
        "- 聚合与分母守恒: 5m/2m/1m 均通过，分别使用本层 bin size 的 diagonal nuisance 层。",
        "- C0 固定状态 value/gradient 等价、C1 精确扣 bend、C3 仅 exposure 变化、C2 map/free domain hook 与 writer 检查: 全部通过。",
        "- C2-free 的半径 1.2 点经合法 pack/evaluate/writer 保留，未套用 sphere inverse、clip 或 rescale。",
        "",
        "## 数值 gate",
        "",
        "- C0 六个 stage 全部有终态记录、checkpoint、入口/终点 physical state 与 020 历史记录逐 stage 比较。",
        "- state gate 要求 byte-identical；数值字段使用 abs `1e-12` / rel `1e-10`；elapsed 秒数不作复现判定。",
        "- gate 状态: `%s`；差异条数: `%d`。" % (status, len(gate.get("differences", []))),
        "- 选中结果: `%s`；新旧最终 count NLL 差最大绝对值 `%.17g`。" % (
            run_result.get("selected_id"), float(gate["selection"]["score_max_abs"])),
        "",
        "## 产物",
        "",
        "- `controlled_protocol.json`：完整 30-attempt 冻结计划与 C0 放行门。",
        "- `preflight.json`：无优化针对性预检证据。",
        "- `C0_gate.json`：无 reference/phase 的历史数值复现 gate。",
        "- `terminal_evidence.json`：六个 stage 的 raw solver、stop reason、退出状态与 hashes。",
        "- `selection.json`：label-free 候选选择；`selected.3dg` 仅为训练输出，不代表评价结论。",
        "",
        "本报告不读取或生成 reference、phase、evaluation、R2 数据；预算结束被记录为 solver 未收敛/预算终止时，不解释为收敛成功。",
    ]
...
def run_c0(outdir: str | os.PathLike[str], *, workers: int = 2, threads: int = 1) -> dict[str, Any]:
    """Freeze and execute exactly the two C0 candidates across three layers."""
    if int(workers) != 2:
