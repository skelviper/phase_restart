"""Phase-free warm continuation of the released 020 random_joint 1 Mb fit.

This runner is intentionally one-candidate and training-only.  It imports only the
frozen contact objective and SciPy optimizer, never evaluation loaders or native FDG.
A checkpoint contains theta/y/coordinates plus a local and cumulative iteration audit;
there is no attempt to recreate L-BFGS memory from the parent run.
"""
...
class ContinuationError(RuntimeError):
    """Raised when a frozen continuation invariant is violated."""
...
def _dependency_snapshot(outdir: Path) -> dict[str, Any]:
    ...
    except Exception as exc:  # pragma: no cover - scipy is required by the runner
...
def _prepare(outdir: Path, parent_config: Mapping[str, Any], start: Mapping[str, Any]) -> dict[str, Any]:
    ...
    readme = """# 022 V1 Continuation Training\n\nThis is the phase-free training record for a warm restart from the released 020 `random_joint` 1 Mb endpoint. The checkpoint contains theta/y/coordinates but no L-BFGS memory, so the solver restart is explicit in every progress row and checkpoint. The run uses the unchanged V1 full-grid contact model on all 1,703,888 SNP-free records for exactly the registered 240-step continuation budget.\n\nThe training runner does not open phase-bearing input, reference 3DG, oracle/softall coordinate payloads, or native FDG. It does not perform FDG or R2 evaluation. The independent FDG/R2 branch receives the final coordinate path only after this directory is hash-audited.\n\nSee `config.json`, `protocol-freeze.json`, `preflight.json`, `logs/progress.jsonl`, `checkpoints/`, `final_theta.npz`, and `terminal_audit.json`.\n"""
