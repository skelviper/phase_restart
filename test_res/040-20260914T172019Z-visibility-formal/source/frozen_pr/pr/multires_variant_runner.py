"""Independent multiresolution controller for post-020 allele variants.

The C0 reproduction remains owned by ``pr.c0_controlled_runner``.  This module
only prepares and, after an explicit C0 gate, runs C1/C2-map/C2-free/C3.  Each
variant/source pair owns a complete 5 Mb -> 2 Mb -> 1 Mb chain.  No evaluator,
phase-bearing input, reference coordinate, or native FDG path is imported.
"""
...
class VariantRunnerError(RuntimeError):
    """Raised when a frozen variant or release contract is violated."""
...
def _write_atomic_json(path: Path, value: Any) -> str:
    """Replace mutable status metadata without weakening exclusive artifacts."""
    path.parent.mkdir(parents=True, exist_ok=True)
    payload = _json_bytes(value)
...
def transfer_layer(coords: np.ndarray, positions: np.ndarray,
    ...
    """Copy the frozen warm-start interpolation, with explicit free-domain handling.

    The interpolation, duplicate handling, noise allocation, and seed are the
    same operations as ``reconstruction_init.warm_start_from_layer``.  Only its
    final radial operation differs: bounded models use the registered 1-1e-6
    clip; C2-free deliberately performs no sphere inverse, clip, or rescale.
    """
...
def _fit_free_domain_aware(objective: contact_model.JointObjective, initial_y: np.ndarray,
    ...
    """The frozen ``joint_fit`` loop without its bounded-only final assertion."""
...
def _fit_layer(root: Path, context: reconstruct.RunContext, model_id: str,
    ...
    if fit_callable is not None:
        ...
        # C0/C1/C2-map/C3 retain the exact original fit_joint entry point.
...
def select_variant_candidate(candidates: Sequence[Mapping[str, Any]],
    ...
    """Select within one variant by count NLL only, with the frozen tie order."""
    usable = []
...
def run_variants(outdir: str | os.PathLike[str], *, c0_gate: str | os.PathLike[str],
    ...
    """Run the remaining 24 stages only after the fixed 033 C0 gate passes."""
...
def _real_layer_probe(context: reconstruct.RunContext) -> dict[str, Any]:
    """Validate each real aggregate layer without constructing an objective fit."""
    rows = {}
    started = time.perf_counter()
    for stage in STAGES:
        data = contact_model.load_frozen_p9016_aggregate(stage.bin_size)
...
def run_preflight(context: reconstruct.RunContext, root: Path) -> dict[str, Any]:
    """Run only synthetic/reference-free controller checks; never optimize."""
...
def prepare(outdir: str | os.PathLike[str], formal_out: str | os.PathLike[str],
            *, workers: int = 4, threads: int = 1) -> dict[str, Any]:
    """Create immutable preparation evidence without launching any variant stage."""
    if int(workers) < 1 or int(workers) > 4:
